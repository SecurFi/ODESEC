...
async def verify_file(verify_bin: str, proof: str, rpc_url: str):
    args = [verify_bin, "verify", "-r", rpc_url, str(proof)]
...
async def handle_file(message: telebot.types.Message):
    ...
    await bot.edit_message_text(
        ...
        text="<i>Verifying your proof...</i>",
        parse_mode="HTML",
    )
    # bot.reply_to(message, "Document received")
    logger.info(
        "receiv a new proof from user %s, name: %s",
    ...
    await bot.send_message(
        message.chat.id, "Please make a short description of the emergency."
    )
    # async with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
...
async def handle_description(message: telebot.types.Message):
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="Thank you for your submission!, The project team will contact you as soon as possible.",
    )
...
async def update_odesec_projects():
    ...
    odesec_contract = w3.eth.contract(address=ODESEC_CONTRACT_ADDRESS, abi=ODESEC_ABI)
    ...
    new_projects = await odesec_contract.functions.getProjectList(
        10, len(ALL_PROJECTS)
    ).call()
